...
*Setup instructions for a gitian build of yotoken using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the yotoken
...
- [Building yotoken](#building-yotoken)
...
Clone the git repositories for yotoken and gitian and then checkout the yotoken version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yotokencoin/yotoken-core.git
cd yotoken-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the yotoken-core repository
...
To build yotoken (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the yotoken-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yotoken-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/yotokencoin/yotoken-core
...
./bin/gbuild --commit yotoken=${COMMIT} --url yotoken=${URL} ../yotoken/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yotoken=${COMMIT} --url yotoken=${URL} ../yotoken/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yotoken=${COMMIT} --url yotoken=${URL} ../yotoken/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yotoken-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yotoken-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/yotoken-build.assert
```
...
[yotokencoin/gitian.sigs](https://github.com/yotokencoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
